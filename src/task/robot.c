...
#include "supercap/supercap_task.h"
...
rt_thread_t supercap_thread_handle;
...
#ifdef BSP_USING_SUPERCAP_TASK
    /* 创建线程，名称是  supercap ，入口是  supercap_thread_entry */
    supercap_thread_handle = rt_thread_create("supercap",
                                             supercap_thread_entry, RT_NULL,
                                             1024,15, 10);
    /* 如果获得线程控制块，启动这个线程 */
    if (supercap_thread_handle != RT_NULL)
        rt_thread_startup(supercap_thread_handle);
#endif /* BSP_USING_SUPERCAP_TASK*/
