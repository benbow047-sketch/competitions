...
static publisher_t *pub_shoot;
 subscriber_t *sub_cmd, *sub_referee;
 subscriber_t *sub_barrel;
...
pid_obj_t *barrel_pid; // 枪管速度PID
int16_t now_output = 0;
int16_t last_output = DBUS_FRICTION_LAUNCH_SPEED;
...
/*----------------------射击状态初始化----------------------------------*/
    ...
    for (;;)
    {
        ...
        {
            rt_pwm_set(servo_cover_dev, PWM_COVER_CH, 20000000, 10000000);
        }

        else
        {
            rt_pwm_set(servo_cover_dev, PWM_COVER_CH, 20000000, 1300000);
        }
       ...
       /* if (sht_motor[TRIGGER_MOTOR]->measure.real_current>=8000||reverse_cnt!=0)
        ...
        {
            shoot_motor_ref[RIGHT_FRICTION] = -last_output;
            shoot_motor_ref[LEFT_FRICTION] = last_output;
        }
        else
        {
            shoot_motor_ref[TRIGGER_MOTOR] = 0;
...
/* ------------------------------------------------  拨弹电机------------------------------------------------------------------------- */
    ...
    pid_config_t barrel_pid_config = INIT_PID_CONFIG(BARREL_KP, BARREL_KI, BARREL_KD, BARREL_INTEGRAL, BARREL_MAX,
                                                        (PID_Trapezoid_Intergral | PID_Integral_Limit | PID_Derivative_On_Measurement));
    barrel_pid= pid_register(&barrel_pid_config);
/* ---------------------------------- shoot电机初注册---------------------------------------------------------------------------------------- */
...
static void shoot_sub_pull()
{
    ...
    sub_get_msg(sub_referee, &referee_data);
}
